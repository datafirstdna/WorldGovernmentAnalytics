  ...
  d3.csv("data/Upwork Dataset-4.csv").then(function (dataCsv) {
    ...
    d3.json("data/WORLDCOUNTRIES.geojson").then(function (data) {
      ...
      map.on("click", "states", function (e) {
        ...
        var isoA2 = callP.ISO_A2;

        var infoShow = ["<img class='propPopMicon' src='https://raw.githubusercontent.com/stevenrskelton/flag-icon/master/png/75/country-4x3/" + isoA2.toLowerCase() + ".png' alt=''></img>"];
          ...
          if (keysN == "") {
            ...
            if (i == 3) {
              ...
              if (split[1] == gloYear || i < 3) {
                if (split[0] == "indicator") {
                  var label = "Country";
                } else {
                  var label = split[0];
                }

                var info = "<h2 class='popA'>" + label + "</h2>" + "<h2 class='popB'>" + inside + "</h2>";
                infoShow.push(info);
              }
            }
          }
      ...
      function updatePanel() {
        ...
        data.features.map(function (d) {
          ...
          if (val != "nd") {
            UNSORTED.push({ admin: d.properties["indicator_data year"], number: val, isoa2: d.properties["ISO_A2"].toLowerCase(), isoa3: d.properties["ISO_A3"] });
        ...
        if (max == null) {
          ...
          document.getElementById("l1").innerHTML = formatNum(minT, 1);
          document.getElementById("l2").innerHTML = formatNum(maxT, 1);

          document.getElementById("legend-grad").style.display = "block";
          document.getElementById("first3-tot").style.display = "block";

          ["match", ["get", "class"], ["airport", "piste"], "hsl(345, 6%, 87%)", ["hospital"], "hsl(0, 56%, 89%)", "hsla(0, 0%, 0%, 0)"];

          var srcLink = "https://raw.githubusercontent.com/stevenrskelton/flag-icon/master/png/75/country-4x3/";

          var filterTop = ["in", "ISO_A3"];
          var filterBottom = ["in", "ISO_A3"];
          ...
          for (i = 0; i < 3; i++) {
            ...
            document.getElementById("first3-flag-" + i).src = srcLink + sorted[i]["isoa2"] + ".png";
            //BOTTOM3
            document.getElementById("first3-label-" + i + "b").innerHTML = sorted[sorted.length - 1 - i]["admin"];
            document.getElementById("first3-val-" + i + "b").innerHTML = formatNum(sorted[sorted.length - 1 - i]["number"], 2);
            document.getElementById("first3-flag-" + i + "b").src = srcLink + sorted[sorted.length - 1 - i]["isoa2"] + ".png";
          }

          if (filterTop != null) {
            for (i = 0; i < 3; i++) {
              filterTop.push(sorted[i]["isoa3"]);
              filterTop.push(sorted[sorted.length - 1 - i]["isoa3"]);
            }
            console.log(filterTop);
            map.setFilter("boundariesTop", filterTop);
            //map.setFilter("boundariesBottom", filterBottom);
          }

          var filterColor = [
            "match",
            ["get", "ISO_A3"],
            [sorted[0]["isoa3"], sorted[1]["isoa3"], sorted[2]["isoa3"]],
            "#fff",
            [sorted[sorted.length - 1]["isoa3"], sorted[sorted.length - 2]["isoa3"], sorted[sorted.length - 3]["isoa3"]],
            "#000",
            "#F3F3F3",
          ];
          ...
          map.setPaintProperty("boundariesTop", "line-color", filterColor);
        }
      }
      ...
      var narrative = [
        ...
        "Highest countries affected by <span class='nar-bold'>maternal mortality</span> are African countries and bottom in place are European regions.",
        " Highest countries affected by <span class='nar-bold'>infant mortality</span> are African countries and bottom in place are European regions.",
      ...
      var availableYears = [2017, 2018, 2019, 2020, 2021];
      ...
      $(".sele").on("click", function (e) {
        ...
        availableYears = [];
        var availableYearsTOT = [];

        for (i = 2014; i < 2023; i++) {
          csvKeys.forEach(function (item) {
            if (gloParam + "_" + i == item) {
              availableYears.push(i);
              availableYearsTOT.push(gloParam + "_" + i);
            }
          });
        }
        availableYears = availableYears.sort();
        console.log(availableYears);
        console.log(availableYearsTOT);

        var yea = availableYears[availableYears.length - 1];
        gloYear = yea;

        console.log([availableYears.length - 1]);

        document.getElementById("slider").min = 0;
        document.getElementById("slider").max = [availableYears.length - 1];
        document.getElementById("slider").value = [availableYears.length - 1];

        document.getElementById("year").textContent = "YEAR " + yea;
      ...
      document.getElementById("slider").addEventListener("input", function (e) {
        ...
        sliderCount++;
        gloYear = availableYears[getVal];
        console.log(availableYears);
        console.log(gloYear + "____" + getVal);
        console.log(sliderCount);

        var startingTitle = document.getElementById("dropdownTitle").innerHTML;
        if ((sliderCount == 1) & (startingTitle == "Visualize Country by...")) {
          document.getElementById("narrativeSel").innerHTML = narrative[17];
